...
import createICAPandaDoc from '@salesforce/apex/PandaDocGeneratorICATemplate.createICAPandaDoc';
import sendDocument from '@salesforce/apex/PandaDocGeneratorICATemplate.sendDocument';
import getContactName from '@salesforce/apex/PandaDocGeneratorICATemplate.getContactName';
...
export default class PandadocsDocumentGeneratorICATemplate extends LightningElement {
    @track documentList;
    @track error;
    ...
    connectedCallback() {
        ...
        setTimeout(() => {
            this.getContactNameHelper();
        }, 50);
    }

    getContactNameHelper(){
        ...
        getContactName({recordId: this.recordId})
			.then( (documentName) => {
				...
				this.documentName = 'ICA - ' + documentName;
    ...
    createDocumentHelper(sendDocument){
        this.spinnerStatus = true;
        createICAPandaDoc({ recordId: this.recordId})//, documentName : this.documentName
			.then( (result) => {
                if(result){
                    const event = new ShowToastEvent({
